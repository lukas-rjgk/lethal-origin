...
class Talk(object):
    ...
    def sendMessage(self, to, text, contentMetadata={}, contentType=0,msgid=None):
        #msg = self.generateReplyMessage(relatedMessageId)
        msg = Message()
        if 'MENTIONnn' in contentMetadata.keys()!=None:
            try:
        ...
        if to not in self._messageReq:
            self._messageReq[to] = -1
        self._messageReq[to] += 1
        a = self.talk.sendMessage(self._messageReq[to], msg)
    ...
    def datamention(self, to, text, data, ps=''):
        ...
        for aa in range(k+1):
            ...
            self.sendMention(to, msgas, data[aa*20 : (aa+1)*20])
    ...
    @loggedIn
    def sendSticker(self, to, packageId, stickerId, text='', meta=None):
        contentMetadata = {
            'STKVER': '100',
            'STKPKGID': packageId,
            'STKID': stickerId
        ...
        if meta != None:
            contentMetadata.update(meta)

        return self.sendMessage(to, text, contentMetadata, 7)
    ...
    def findGroupByTicket(self, ticketId):
        ...
        elif app_name in ["DESKTOPWIN", "DESKTOPMAC", "IOSIPAD", "CHROMEOS", "ANDROIDLITE"]:
